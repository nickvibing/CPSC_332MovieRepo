...
SELECT
    ...
    s.start_dt,
    m.title,
    COUNT(DISTINCT CONCAT(se.row_label, se.seat_no)) AS total_seats,
...
SELECT
    th.theatre_id,
    th.name AS theatre_name,
    COUNT(DISTINCT CONCAT(se.showtime_id, ':', se.row_label, ':', se.seat_no)) AS total_seat_instances,
    SUM(CASE WHEN t.state = 'PURCHASED' THEN 1 ELSE 0 END)   AS seats_sold,
    CASE
        WHEN COUNT(DISTINCT CONCAT(se.showtime_id, ':', se.row_label, ':', se.seat_no)) = 0
        THEN 0
        ELSE ROUND(
            100.0 * SUM(CASE WHEN t.state = 'PURCHASED' THEN 1 ELSE 0 END)
            / COUNT(DISTINCT CONCAT(se.showtime_id, ':', se.row_label, ':', se.seat_no)),
            2
        )
